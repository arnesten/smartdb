...
function smartdb(createOptions) {
    ...
    let validate = promisify(createOptions.validate || ((entity, callback) => { callback(); }));
    ...
    createOptions.databases.forEach(databaseInfo => {
        ...
        Object.keys(databaseInfo.entities).forEach(entityType => {
            ...
            let cache = cacheProvider.create(entityType, entitySettings);
            entityInfoMap[entityType] = {
                databaseUrl: databaseUrl,
                safeDatabaseUrl: safeDatabaseUrl,
                nanoDb: Object.assign(nanoDb, {
                    get: promisify(nanoDb.get),
                    insert: promisify(nanoDb.insert),
                    destroy: promisify(nanoDb.destroy),
                    view: promisify(nanoDb.view),
                }),
                cache: Object.assign(cache, {
                    get: promisify(cache.get),
                    set: promisify(cache.set),
                    del: promisify(cache.del),
                })
            };
        });
    });
    ...
    return {
        view: callbackify(view),
        viewValue: callbackify(viewValue),
        viewRaw: callbackify(viewRaw),
        list: callbackify(list),
        get: callbackify(get),
        getOrNull: callbackify(getOrNull),
        save: callbackify(save),
        update: callbackify(update),
        updateWithRetry: updateWithRetry,
        merge: callbackify(merge),
        remove: callbackify(remove)
    };

    async function view(type, viewName, args) {
        if (typeof type !== 'string') throw new Error('type required');
        if (typeof viewName !== 'string') throw new Error('viewName required');
    ...
    async function list(type, listName, viewName, args) {
        if (typeof type !== 'string') throw new Error('type required');
        if (typeof listName !== 'string') throw new Error('listName required');
        if (typeof viewName !== 'string') throw new Error('viewName required');
        if (!args) throw new Error('args required');
    ...
    async function save(entity) {
        ...
        try {
            let promise = docId ?
                entityInfo.nanoDb.insert(doc, docId) :
                entityInfo.nanoDb.insert(doc);
    ...
    async function update(entity) {
        ...
        let docId = doc._id;
        delete doc._id;
        await entityInfo.cache.del(docId);

        let response = await entityInfo.nanoDb.insert(doc, docId).catch(err => {
            fixNanoError(entityInfo, err);
            if (err.statusCode === 409) {
                throw entityConflictError(err, {
                    entityId: docId,
    ...
    async function remove(type, id) {
        ...
        let result = await entityInfo.nanoDb.destroy(id, entity._rev).catch(err => {
            fixNanoError(entityInfo, err);
            if (err.statusCode === 409) {
                throw entityConflictError(err, {
                    entityId: id,
