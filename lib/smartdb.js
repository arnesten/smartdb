...
function smartdb(createOptions) {
    ...
    let validate = createOptions.validate || ((entity, callback) => { callback(); });
    let validatePromise = promisify(validate);
    ...
    let getPromise = promisify(get);
    let updatePromise = promisify(update);
    return {
        view: promisify(view),
    ...
    async function save(entity) {
        ...
        try {
            let promise = docId ?
                insertPromise(entityInfo.nanoDb, doc, docId) :
                insertPromise(entityInfo.nanoDb, doc);
    ...
    function insertPromise(nanoDb, ...args) {
        return new Promise((resolve, reject) => {
            nanoDb.insert(...args, (err, response) => {
                if (err) {
                    reject(err);
                }
                else {
    ...
    async function updateWithRetry(type, id, updateFunction) {
        ...
        try {
            let entity = await getPromise(type, id);
            await updateFunction(entity);
            await updatePromise(entity);
            return entity;
        }
        catch (err) {
            let entity = await getPromise(type, id);
    ...
    function hookPromise(event, entityInfo) {
        return new Promise((resolve, reject) => {
            hook(event, entityInfo, err => {
                if (err) {
                    reject(err);
                }
                else {
