...
function SmartDb(createOptions) {
    ...
    async function find(type, indexName, args) {
        ...
        let findArgs = { ...args };
        findArgs.selector = { ...args.selector, use_index: indexName };

        let { docs } = await entityInfo.nanoDb.find(findArgs);

        return docs.map(doc => docToEntity(doc));
    }
    ...
    async function getOrNullBulk(type, ids) {
        ...
        await Promise.all(rows
            .map(row => row.doc)
            .filter(doc => doc && doc[typeProperty] === type)
            .map(doc => cache.set(doc._id, doc)));
